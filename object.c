...
ActiveField * get_static_field(ClassFile * class_file, char * field_name, char * field_descriptor) {
    for (int i = 0; i < class_file->static_fields_count; i++) {
        char * name = (char *) class_file->constant_pool[class_file->static_fields[i]->field->name_index - 1]->Utf8.bytes;
        if (strcmp(name, field_name) == 0) {
            char * descriptor = (char *) class_file->constant_pool[class_file->static_fields[i]->field->descriptor_index - 1]->Utf8.bytes;
...
int getstatic(Frame * frame, Instruction instruction) {
    ...
    char * field_descriptor = (char *) frame->this_class->constant_pool[nameandtype->NameAndType.descriptor_index - 1]->Utf8.bytes;
...
ActiveField * get_field(Object * object, char * field_name, char * field_descriptor) {
    for (int i = 0; i < object->class->fields_count; i++) {
        char * name = (char *) object->class->constant_pool[object->fields[i]->field->name_index - 1]->Utf8.bytes;
        if (strcmp(name, field_name) == 0) {
            char * descriptor = (char *) object->class->constant_pool[object->fields[i]->field->descriptor_index - 1]->Utf8.bytes;
            if (strcmp(descriptor, field_descriptor) == 0) {
